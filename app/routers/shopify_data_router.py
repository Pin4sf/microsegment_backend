...
async def _handle_shopify_request(request_data: ShopifyDataRequest, fetch_method_name: str, response_model, **kwargs):
    """Helper function to handle Shopify data requests to reduce boilerplate."""
    try:
        client = ShopifyClient(shop=request_data.shop,
        ...
        if raw_response and "data" in raw_response:
            ...
            if fetch_method_name == "get_products" and "products" in data_content:
                return ShopifyProductResponse(products=data_content["products"], success=True, message="Products fetched successfully.")
            elif fetch_method_name == "get_orders" and "orders" in data_content:
                return ShopifyOrderResponse(orders=data_content["orders"], success=True, message="Orders fetched successfully.")
            elif fetch_method_name == "get_customers" and "customers" in data_content:
                return ShopifyCustomerResponse(customers=data_content["customers"], success=True, message="Customers fetched successfully.")
            elif fetch_method_name == "get_transactions" and "node" in data_content and data_content["node"] and "transactions" in data_content["node"]:
                # Transactions are now a direct list
                return ShopifyTransactionResponse(transactions=data_content["node"]["transactions"], success=True, message="Transactions fetched successfully.")
            elif fetch_method_name == "get_transactions" and "node" in data_content and data_content["node"] is None:
                logger.warning(
                    f"Order node not found for ID {kwargs.get('order_id')} when fetching transactions.")
                return response_model(success=False, message=f"Order with ID {kwargs.get('order_id')} not found or has no transactions.", data=None)
            else:
                ...
                return response_model(success=False, message="Data fetched but structure was unexpected.", data=data_content)
            ...
            return response_model(success=False, message=f"GraphQL error: {raw_response['errors'][0]['message']}", data=raw_response)
        else:
            logger.error(
                f"No data or unexpected response from {fetch_method_name} for shop {request_data.shop}")
            return response_model(success=False, message="Failed to fetch data from Shopify or empty response.", data=raw_response)
        ...
        logger.error(
            f"HTTPStatusError in {fetch_method_name} for shop {request_data.shop}: {hse.response.text}", exc_info=True)
        raise HTTPException(status_code=hse.response.status_code,
                            detail=f"Shopify API error: {hse.response.text}")
    except Exception as e:
        logger.error(
            f"Unexpected error in {fetch_method_name} for shop {request_data.shop}: {e}", exc_info=True)
        raise HTTPException(
            status_code=500, detail=f"An unexpected error occurred while fetching {fetch_method_name.split('_')[-1]}.")


@router.post("/products", response_model=ShopifyProductResponse, summary="Fetch Shopify Products")
async def get_shopify_products(request_data: ShopifyDataRequest = Body(...)):
    ...
    """
    return await _handle_shopify_request(request_data, "get_products", ShopifyProductResponse)


@router.post("/orders", response_model=ShopifyOrderResponse, summary="Fetch Shopify Orders")
async def get_shopify_orders(request_data: ShopifyDataRequest = Body(...)):
    ...
    Requires shop domain, access token, and supports pagination (cursor, first) and custom query filters.
    """
    return await _handle_shopify_request(request_data, "get_orders", ShopifyOrderResponse)


@router.post("/transactions", response_model=ShopifyTransactionResponse, summary="Fetch Shopify Transactions for an Order")
async def get_shopify_transactions(request_data: ShopifyTransactionRequest = Body(...)):
    ...
    """
    return await _handle_shopify_request(request_data, "get_transactions", ShopifyTransactionResponse, order_id=request_data.order_id)


@router.post("/customers", response_model=ShopifyCustomerResponse, summary="Fetch Shopify Customers")
async def get_shopify_customers(request_data: ShopifyDataRequest = Body(...)):
    ...
    Requires shop domain, access token, and supports pagination (cursor, first) and custom query filters.
    """
    return await _handle_shopify_request(request_data, "get_customers", ShopifyCustomerResponse)
